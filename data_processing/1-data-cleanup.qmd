...
The engtxt column is not actually different from the review column, the only difference being conversions of any reviews that just say "xx%" (100% being converted to 1) or anything with x/10 being also converted to a string of numbers. Since it's essentially the same data, I'll ditch the engtxt column and keep the review one.
...
Now, I wanted to verify everything is correct. If every review ID is unique, then piping the review_id column into distinct() should be the exact same dimensions as the full dataframe.
...
But it's not... so an investigation is needed
...
It looks like, for some reason, the game Superpower 3 has the same reviews as the command & conquer game. I went to check into this more, and it looks like, for some reason, the Superpower 3 game ID didn't actually work with the script to rip game reviews and I missed it by mistake. I won't work now, either, so I'm going to have to find a replacement game in the overwhelmingly negative category and rebuild my dataframe. It's only the one, though, not the end of the world. 


```{r}
#| label: dupe-check
reviews_df |>
  filter(review_id==207594063)
```
