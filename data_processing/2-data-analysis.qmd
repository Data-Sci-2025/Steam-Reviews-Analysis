...
Move the measurement stuff up here that first showed the hugely disparate numbers of both reviews per games and positive vs negative reviews. 

Use the table in the questions menu below but merge it with the games info too to include the game's rank to prove the point.

Positively reviewed games have more reviews than negatively reviewed games quite significantly. To account for this I decided to downsample those positively reviewed games 


```{r}
simple_rank <- merged_games |>
  group_by(rank) |>
  ...
  mutate(rank_simp = str_replace_all(rank, "\\w+ (\\w)", "\\1"))
...
```{r}
other_games <- simple_rank |>
  filter(rank_simp!='Positive')
```



```{r}
posgames <- simple_rank |>
  filter(rank_simp=="Positive")
...
```{r}
#| eval: FALSE
sampled_data <- posgames |>
  group_by(your_stratification_variable) %>%
...
set seed in R - set.seed(1234) on the downsampling code so other people get the same sampling

slice_sample - stratified downsampling

1. Should I cap number of reviews per game to even things out? YES - DOWNSAMPLING
